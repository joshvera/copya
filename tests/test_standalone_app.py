...
class StandaloneAppTest(unittest.TestCase):
    ...
    def test_build_and_package_scripts_do_not_require_pyinfra_or_jinja(self) -> None:
        ...
        self.assertIn("attached_device", package_script)
        self.assertIn("canonical_mount_dir", package_script)
        self.assertIn("detach_image", package_script)
        self.assertIn("hdiutil detach \"$target\" -force", package_script)
        self.assertIn("leaving $work_dir for manual cleanup", package_script)
